...
@login_required(login_url='/managerlogin/')
def LeadsToEstimates(request,id):
    objs = Request.objects.get(id = id)
    estimates = Estimate.objects.all().last()
    useadd = UserAddress.objects.filter(profile = objs.profile).last()
    print(useadd)
    if estimates:
        num = estimates.estimate_number
        estimate_number = int(num.split('-')[1])
        # print(estimate_number)
        context = {"objs":objs,"estimate_number":estimate_number+1,'useadd':useadd}
    else:
        context = {"objs":objs,"estimate_number":1,'useadd':useadd}
    return render(request,"leadstoestimate.html",context)
...
def ManagersView(request):
    if request.user.id:
        ...
        if not admin.is_staff:
            return HttpResponse('You are not Superadmin')
    managers = Managers.objects.all()
    context = {'managers':managers}
    if request.method == "POST":
        data = request.POST
        deleteit = data.get('id')
        manager = Managers.objects.get(id = deleteit)
        manager.delete()
...
def AddManagers(request):
    ...
    if request.method == "POST":
        ...
        # Validate for duplicate email and phone number
        if CustomUser.objects.filter(email=data.get('email')).exists():
            return HttpResponse("Error: Email already exists")
        if CustomUser.objects.filter(phone_number=data.get('phone1')).exists():
            return HttpResponse("Error: Phone number already exists")
        ...
        manager = Managers(
            ...
            password=make_password(data.get('name')),  # Hash the password
        )
        manager.save()
        logger.info(f"Manager created: {manager}")
        ...
        user.set_password(data.get('phone1'))  # Use a secure password
        user.save()
        logger.info(f"CustomUser created: {user}")
    ...
    if request.method == "POST": 
        ...
        if identity_value:
            ...
            if user.manager == True:
                print("Logging in user:", user.manager)
                login(request, user)
...
from krupa.utils import send_email_token
@login_required(login_url='/managerlogin/')
...
def save_pdf0(params: dict):
    template = get_template("Einvoice.html")
...
def Newestimates(request):   # sourcery skip: avoid-builtin-shadow, use-named-expression
    ...
    if request.method == "POST":
        try:
            ...
            # send_email_token(customer_name1,customer_name1.id)
...
def save_pdf(params: dict):
    template = get_template("Sinvoice.html")
    html = template.render(params)
    response = BytesIO()
    pdf = pisa.pisaDocument(BytesIO(html.encode('UTF-8')), response)
    file_name = f"{uuid.uuid4()}.pdf"
    ...
    if request.method == "POST":
        try:
            ...
            sales_order = SalesOrder.objects.create(
                ...
                date=data.get('salesOrderDate', None),
                expiry_date=data.get('expectedShipmentDate', None),
...
# sourcery skip: merge-nested-ifs
            ...
            customer_name1.request = True
            customer_name1.save()
            # customer_name1.estimate = estimate
...
def save_pdf2(params: dict):
    template = get_template("Iinvoice.html")
    html = template.render(params)
    response = BytesIO()
    pdf = pisa.pisaDocument(BytesIO(html.encode('UTF-8')), response)
    file_name = f"{uuid.uuid4()}.pdf"
...
def Invoice3(request):  # sourcery skip: low-code-quality, use-named-expression
    ...
    if request.method == "POST":
        try:
            ...
            try:
                customer_name1 = Request.objects.get(id=id1)
            except Request.DoesNotExist:
                return JsonResponse({'error': 'Customer not found'}, status=404)
...
def save_pdf3(params: dict):
    template = get_template("Pinvoice.html")
    html = template.render(params)
    response = BytesIO()
    pdf = pisa.pisaDocument(BytesIO(html.encode('UTF-8')), response)
    file_name = f"{uuid.uuid4()}.pdf"
...
@login_required(login_url='/managerlogin/')
def AssignedCustomer(request):
    assigned = Request.objects.all()
    support = SupportTicket.objects.all()
    context = {"assigned":assigned,"support":support}
    if request.method == "POST":
        data = request.POST
        id = data.get("id")
        # print(id)
        status = data.get('status')
        update = SupportTicket.objects.get(id = id)
        update.progress = status
        update.save()
    return render(request,"assinged.html",context)